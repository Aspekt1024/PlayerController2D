...
namespace Aspekt.PlayerController
{
    public class WallCheck : PlayerSensor
    {
        ...
        {
            Vector2 rayDirection = new Vector2(player.IsFacingRight() ? 1 : -1, 0);
            RaycastHit2D hit = Physics2D.Raycast(coll.transform.position, rayDirection, coll.bounds.extents.x + 0.02f, 1 << LayerMask.NameToLayer("Terrain"));
